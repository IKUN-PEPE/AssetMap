...
class SaveAssetsResult:
    def __init__(self):
        ...
        self.saved_asset_ids: list[str | None] = []
...
def _save_asset_row_with_session(
    ...
    if hasattr(asset_db, "flush"):
        asset_db.flush()
    return (success, duplicate, 0, observation_only, getattr(web, "id", None) if web is not None else None)
    ...
    *,
    ...
    for index, asset_data in enumerate(assets, start=1):
        ...
        try:
            ...
            success, duplicate, failed, observation_only, saved_asset_id = _save_asset_row_with_session(
                asset_db,
                isolated_job,
                asset_data,
                source_name,
            ...
            result.saved_asset_ids.append(saved_asset_id)
